...
from parsl.launchers import JsrunMPILauncher
...
lassen_htex = HighThroughputExecutor(
    ...
    provider=LSFProvider(
        launcher=JsrunMPILauncher(overrides='-g 1 -a 1'),
        walltime="00:30:00",
        nodes_per_block=1,
        init_blocks=1,
        max_blocks=1,
...
@bash_app(executors=['lassen_htex'])
def run_mirge_again(execution_string='', stdout='run.stdout', stderr='run.stderr', inputs=[], outputs=[]):
    return(execution_string)
...
# first run is just an init, on lassen batch
init_restart_file = File(os.path.join(os.getcwd(), 'restart_data/flame1d-000000-0000.pkl'))
intro_str = 'echo "Running flame1d_init again"\n'
conda_str = load_conda()
input_file = File(os.path.join(os.getcwd(), 'run1_params.yaml'))
execution_str = ('python -u -m mpi4py flame_init.py -i {input_file}\n').format(input_file=input_file)
ex_str = intro_str+conda_str+execution_str
stdout_str = 'flame1d-2_init.stdout'
stderr_str = 'flame1d-2_init.stdout'
flame_init2 = run_mirge_again(execution_string=ex_str, stdout=stdout_str, stderr=stderr_str, outputs=[init_restart_file])

#print(flame_init.outputs)
print('Done: {}'.format(flame_init2.done()))
print('Result: {}'.format(flame_init2.result()))
print('Done: {}'.format(flame_init2.done()))
